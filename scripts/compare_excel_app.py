...
def readiness(version, contract, source_contract_number=None):
    ...
    if version.get("dataMode") != "imported":
        issues.append("сохранённая версия имеет режим demo, а не подтверждённые исходные данные")
...
def report(cells, result, columns, issues, source, saved, contract, version, data):
    lines = ["# Предварительная помесячная сверка Excel и приложения — 2026",
             "", "Статус: **исходные данные не сопоставлены; числовая эквивалентность не подтверждена**.",
    ...
    lines += ["- Выполнение BOQ/КСГ и остальные входные ряды обеих моделей не выверены по документам.",
              "- Денежный поток имеет разный состав: в Excel отдельные гарантийные суммы и НДС; приложение использует упрощённую схему поступлений, выплат и НДС.",
    ...
    lines += ["## Порядок закрытия расхождений", "",
              "1. Подтвердить договор, валюту/единицу и валютный курс на даты расчёта; заменить демонстрационную версию подтверждёнными входными рядами.",
