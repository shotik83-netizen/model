...
process.stdout.write(JSON.stringify({status:aggregate.status,year:aggregate.year,currency:aggregate.currency,
 ...
 individual:entries.map(row=>({number:row.number,revenue:row.result.revenue,costs:row.result.costs,ncf:row.result.ncf,cashBridge:row.result.cashBridge,accepted:row.result.accepted,receipts:row.result.receipts,receivable:row.result.receivable,offsets:row.result.offsets,retention:row.result.retention,factoring:row.result.factoring,deductions:row.result.deductions}))}));
